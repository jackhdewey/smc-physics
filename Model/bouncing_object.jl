...
function init_scene(debug_viz::Bool, model_args::Args)
...
function init_target_state(sim::PhySim, shape::String, init_position::Vector{Float64}, init_velocity::Vector{Float64}, mass::Float64=1.0, restitution::Float64=0.9)
    ...
    if (shape === "Cube")
        ...
        body = bullet.createCollisionShape(bullet.GEOM_BOX, halfExtents=[0.05, 0.05, 0.05])
    else
        print("Initializing Sphere\n")
        body = bullet.createCollisionShape(bullet.GEOM_SPHERE, radius=0.05)
    end
...
@gen function sample_init_state(k::RigidBodyState, model_args)
...
@gen function resample_state(k::RigidBodyState, transition_noise)
...
# @gen function sample_observation(k::RigidBodyState, observation_noise)

#     obs = {:position} ~ broadcasted_normal(k.position, observation_noise)
#     # obs = broadcasted_normal(k.position, observation_noise)

#     return obs
# end
# function sample_observation_builder(args::Args)
#     observation_noise = args.observation_noise
    # @gen function sample_observation_fn(k)
    @gen function sample_observation(k)
        obs = {:position} ~ broadcasted_normal(k, observation_noise)
    end
    # return sample_observation_fn
# end
...
@gen function kernel(t::Int, current_state::BulletState, sim::BulletSim, model_args::Args)

    # Applies system noise to predicted position
    # noisy_kinematics = {:state} ~ Gen.Map(resample_state)(current_state.kinematics)
    @gen function resample_state_closure(x)
        resample_state(x, model_args.transition_noise)
    end
    noisy_kinematics = {:state} ~ Gen.Map(resample_state_closure)(current_state.kinematics)
    ...
    # println(current_state.kinematics)
    # println(sample_observation(current_state.kinematics))
...
@gen function generate_trajectory(sim::BulletSim, init_state::BulletState, model_args::Args, T::Int)
    ...
    # try
    states = {:trajectory} ~ Gen.Unfold(kernel)(T, init_state, sim, model_args)
    # catch e
