...
function main()
    ...
    for i=3:29
        ...
        CSV.write(string("Tests/Data/Cube_Ela", ela, "_Var", (i % 3 + 1), ".csv"), state)
        
        trajectory_data = DataFrame(x=[], y=[], z=[])
        vel = linear_velocity
        while abs(vel[1]) > 0.01 || abs(vel[2]) > 0.01 || abs(vel[3]) > 0.01
        ...
        CSV.write(string("Tests/Data/Cube_Ela", ela, "_Var", (i % 3 + 1), "_observed.csv"), trajectory_data, transform=truncator)
